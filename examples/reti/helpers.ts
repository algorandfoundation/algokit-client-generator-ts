...
export async function getProtocolConstraints(validatorClient: ValidatorRegistryClient) {
  return new ProtocolConstraints((await validatorClient.compose().getProtocolConstraints({}, {}).simulate()).returns![0])
}
...
export async function getValidatorState(validatorClient: ValidatorRegistryClient, validatorId: number) {
  return new ValidatorCurState(
    (await validatorClient.compose().getValidatorState({ validatorId }, {}).simulate({ allowUnnamedResources: true })).returns![0],
  )
}
...
export async function addStakingPool(
  ...
  const newPoolClient = new StakingPoolClient({ sender: vldtrAcct, resolveBy: 'id', id: poolKey.poolAppId }, context.algod)
...
export async function getPoolInfo(validatorClient: ValidatorRegistryClient, poolKey: ValidatorPoolKey) {
  try {
    const PoolRet = await validatorClient.compose().getPoolInfo({ poolKey: poolKey.encode() }, {}).simulate({ allowUnnamedResources: true })
...
export async function getCurMaxStakePerPool(validatorClient: ValidatorRegistryClient, validatorId: number) {
  try {
    return (await validatorClient.compose().getCurMaxStakePerPool({ validatorId }).simulate({ allowUnnamedResources: true })).returns![0]
  } catch (exception) {
    console.log((exception as LogicError).message)
    throw validatorClient.appClient.exposeLogicError(exception as Error)
  }
...
export async function getStakerInfo(stakeClient: StakingPoolClient, staker: Account) {
  try {
    return StakedInfo.fromValues(
      (await stakeClient.compose().getStakerInfo({ staker: staker.addr }, {}).simulate({ allowUnnamedResources: true })).returns![0],
    )
  } catch (exception) {
    console.log((exception as LogicError).message)
...
export async function getTokenPayoutRatio(validatorClient: ValidatorRegistryClient, validatorId: number) {
  return new PoolTokenPayoutRatio(
    (await validatorClient.compose().getTokenPayoutRatio({ validatorId }, {}).simulate({ allowUnnamedResources: true })).returns![0],
  )
}
...
export async function logStakingPoolInfo(context: AlgorandTestAutomationContext, PoolAppID: bigint, msgToDisplay: string) {
  const firstPoolClient = new StakingPoolClient({ sender: context.testAccount, resolveBy: 'id', id: PoolAppID }, context.algod)
