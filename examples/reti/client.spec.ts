...
import { StakingPoolClient } from './staking.client'
import { ValidatorRegistryClient } from './validator.client'
import {
  addStake,
  addStakingPool,
  addValidator,
...
describe('MultValidatorAddCheck', () => {
  ...
  test('validatorAddTests', async () => {
    ...
    let expectedID = 1
    let validatorId = await addValidator(fixture.context, validatorMasterClient, validatorOwnerAccount, config, validatorMbr)
    ...
    expectedID += 1
    validatorId = await addValidator(fixture.context, validatorMasterClient, validatorOwnerAccount, config, validatorMbr)
    expect(validatorId).toEqual(expectedID)
    expectedID += 1
    validatorId = await addValidator(fixture.context, validatorMasterClient, validatorOwnerAccount, config, validatorMbr)
    expect(validatorId).toEqual(expectedID)
...
describe('StakeAdds', () => {
  ...
  beforeAll(async () => {
    ...
    firstPoolKey = await addStakingPool(fixture.context, validatorMasterClient, validatorId, 1, validatorOwnerAccount, poolMbr, poolInitMbr)
  ...
  test('firstStaker', async () => {
    ...
    const stakeAmount1 = AlgoAmount.MicroAlgos(AlgoAmount.Algos(1000).microAlgos + AlgoAmount.MicroAlgos(Number(stakerMbr)).microAlgos)
    ...
    poolInfo = await getPoolInfo(validatorMasterClient, firstPoolKey)
    expect(poolInfo.totalAlgoStaked).toEqual(BigInt(stakeAmount1.microAlgos - Number(stakerMbr) + stakeAmount2.microAlgos))
    // and global state changed
    validatorGlobalState = await validatorMasterClient.appClient.getGlobalState()
    expect(validatorGlobalState.staked.value).toEqual(stakeAmount1.microAlgos - Number(stakerMbr) + stakeAmount2.microAlgos)
    ...
    const stateData = await getValidatorState(validatorMasterClient, validatorId)
    expect(stateData.numPools).toEqual(1)
    expect(stateData.totalAlgoStaked).toEqual(BigInt(stakeAmount1.microAlgos + stakeAmount2.microAlgos - Number(stakerMbr)))
    expect(stateData.totalStakers).toEqual(1n)
    // and. globally
    validatorGlobalState = await validatorMasterClient.appClient.getGlobalState()
    expect(validatorGlobalState.staked.value).toEqual(stakeAmount1.microAlgos + stakeAmount2.microAlgos - Number(stakerMbr))
  })
  ...
  test('nextStaker', async () => {
    ...
    const stateData = await getValidatorState(validatorMasterClient, validatorId)
    expect(stateData.numPools).toEqual(1)
    expect(stateData.totalAlgoStaked).toEqual(origValidatorState.totalAlgoStaked + BigInt(stakeAmount1.microAlgos - Number(stakerMbr)))
  ...
  test('addThenRemoveStake', async () => {
    ...
    const removeFees = await removeStake(ourPoolClient, stakerAccount, AlgoAmount.MicroAlgos(Number(stakerInfo.balance)))
...
export async function verifyRewardAmounts(
  ...
  for (let i = 0; i < stakersPriorToReward.length; i += 1) {
    ...
    if (timePercentage < BigInt(1000)) {
      // partial staker
      const expectedReward = (BigInt(origBalance) * algoRewardedAmount * BigInt(timePercentage)) / (totalAmount * BigInt(1000))
      consoleLogger.info(
      ...
      const expectedTokenReward = (BigInt(origBalance) * tokenRewardedAmount * BigInt(timePercentage)) / (totalAmount * BigInt(1000))
      consoleLogger.info(
...
describe('StakeWRewards', () => {
  ...
  test('testFirstRewards', async () => {
    ...
    expect(BigInt((await firstPoolClient.appClient.getGlobalState()).lastPayout.value as bigint)).toBeGreaterThan(payoutBefore)
    expect(BigInt((await firstPoolClient.appClient.getGlobalState()).epochNumber.value as bigint)).toEqual(epochBefore + 1n)
  ...
  test('testPartialReward', async () => {
    ...
    consoleLogger.info(`lastPayout:${stakingPoolGS.lastPayout.value}, staker1 entry round: ${staker1Info.entryRound}`)
    ...
    const [newPoolKey] = await addStake(fixture.context, validatorMasterClient, validatorId, partialEpochStaker1, partialStakersAmount, 0n)
    ...
    const [newPoolKey2] = await addStake(fixture.context, validatorMasterClient, validatorId, partialEpochStaker2, partialStakersAmount, 0n)
    ...
    expect(poolInfo.totalAlgoStaked).toEqual(BigInt(stakeAmount1.microAlgos + partialStakersAmount.microAlgos * 2) - 2n * stakerMbr)
...
describe('StakeWTokenWRewards', () => {
  ...
  beforeAll(async () => {
    ...
    rewardTokenID = await createAsset(fixture.context.algod, tokenCreatorAccount, 'Reward Token', 'RWDTOKEN', 100_000, decimals)
    ...
    validatorId = await addValidator(fixture.context, validatorMasterClient, validatorOwnerAccount, validatorConfig, validatorMbr)
  ...
  test('extractRewards', async () => {
    ...
    expect(newStakerBalance.amount).toEqual(origStakerBalance.amount + AlgoAmount.Algos(1190).microAlgos - removeFees)
    // verify that reward token payout came to us
    const assetInfo = await fixture.context.algod.accountAssetInformation(stakerAccounts[0].addr, Number(rewardTokenID)).do()
  ...
  test('testPartialReward', async () => {
    ...
    const stakeAmount2 = AlgoAmount.MicroAlgos(AlgoAmount.Algos(1000).microAlgos + AlgoAmount.MicroAlgos(Number(stakerMbr)).microAlgos)
  ...
  test('reclaimTokenRewards', async () => {
    ...
    const rewardTokenBalance = await fixture.context.algod.accountAssetInformation(pool1Address, Number(rewardTokenID)).do()
    ...
    const ownerTokenBalPre = await fixture.context.algod.accountAssetInformation(tokenCreatorAccount.addr, Number(rewardTokenID)).do()
    ...
    const ownerTokenBal = await fixture.context.algod.accountAssetInformation(tokenCreatorAccount.addr, Number(rewardTokenID)).do()
    expect(ownerTokenBal['asset-holding'].amount).toEqual(ownerTokenBalPre['asset-holding'].amount + Number(sentAmount))
  })
})
...
describe('StakeUnstakeAccumTests', () => {
  ...
  test('stakeAccumTests', async () => {
    ...
    const stakeAmount1 = AlgoAmount.MicroAlgos(AlgoAmount.Algos(2000).microAlgos + AlgoAmount.MicroAlgos(Number(stakerMbr)).microAlgos)
    const [stakedPoolKey] = await addStake(fixture.context, validatorMasterClient, validatorId, stakerAccount, stakeAmount1, 0n)
    ...
    expect(newStakeAccum).toEqual(secondStakeAccum - BigInt(roundsRemaining) * BigInt(AlgoAmount.Algos(50).microAlgos))
    ...
    expect(thirdStakeAccum).toEqual(newStakeAccum - BigInt(roundsRemaining) * BigInt(AlgoAmount.Algos(50).microAlgos))
  })
})
...
describe('TokenRewardOnlyTokens', () => {
  ...
  beforeAll(async () => {
    ...
    rewardTokenID = await createAsset(fixture.context.algod, tokenCreatorAccount, 'Reward Token', 'RWDTOKEN', 100_000, 0)
  ...
  test('extractRewards', async () => {
    ...
    // verify that reward token payout came to us
    const assetInfo = await fixture.context.algod.accountAssetInformation(stakerAccount.addr, Number(rewardTokenID)).do()
    expect(BigInt(assetInfo['asset-holding'].amount)).toEqual(tokenRewardPerPayout)
...
describe('DoublePoolWTokens', () => {
  ...
  beforeAll(async () => {
    ...
    poolClients.push(
      new StakingPoolClient({ sender: validatorOwnerAccount, resolveBy: 'id', id: poolKeys[0].poolAppId }, fixture.context.algod),
    )
    ...
    poolClients.push(
      new StakingPoolClient({ sender: validatorOwnerAccount, resolveBy: 'id', id: poolKeys[1].poolAppId }, fixture.context.algod),
    )
  })
  ...
  test('addStakers', async () => {
    for (let i = 0; i < 2; i += 1) {
      ...
      const stakeAmount = AlgoAmount.MicroAlgos(AlgoAmount.Algos(5000).microAlgos + AlgoAmount.MicroAlgos(Number(stakerMbr)).microAlgos)
  ...
  test('testFirstRewards', async () => {
    ...
    for (let poolIdx = 0; poolIdx < 2; poolIdx += 1) {
      ...
      const tokenRewardForThisPool = (BigInt(tokenRewardPerPayout) * payoutRatio.PoolPctOfWhole[poolIdx]) / BigInt(1_000_000)
  ...
  test('extractRewards', async () => {
    for (let i = 0; i < 2; i += 1) {
      ...
      const origStakerAssetBalance = await fixture.context.algod.accountAssetInformation(stakerAccounts[i].addr, Number(rewardTokenID)).do()
      ...
      expect(BigInt(newStakerBalance.amount)).toEqual(BigInt(origStakerBalance.amount) + stakerInfo.balance - BigInt(removeFees))
      // verify that pending reward token payout came to us
      const newStakerAssetBalance = await fixture.context.algod.accountAssetInformation(stakerAccounts[i].addr, Number(rewardTokenID)).do()
      ...
      expect(postRemovePoolInfo.totalAlgoStaked).toEqual(BigInt(origPoolInfo.totalAlgoStaked - stakerInfo.balance))
      ...
      expect(newValidatorState.rewardTokenHeldBack).toEqual(BigInt(origValidatorState.rewardTokenHeldBack - stakerInfo.rewardTokenBalance))
    }
  })
})
...
describe('TokenGatingByCreator', () => {
  ...
  beforeAll(async () => {
    ...
    gatingToken1Id = await createAsset(fixture.context.algod, tokenCreatorAccount, 'Gating Token 1', 'GATETK1', 10, 0)
    gatingToken2Id = await createAsset(fixture.context.algod, tokenCreatorAccount, 'Gating Token 2', 'GATETK2', 10, 0)
  ...
  describe('stakeTest', () => {
    ...
    beforeAll(async () => {
      ...
      stakerCreatedTokenId = await createAsset(fixture.context.algod, stakerAccount, 'Dummy Token', 'DUMMY', 10, 0)
    })
...
describe('TokenGatingByAsset', () => {
  ...
  beforeAll(async () => {
    ...
    gatingToken1Id = await createAsset(fixture.context.algod, tokenCreatorAccount, 'Gating Token 1 [Other by same]', 'GATETK1', 10, 0)
    gatingToken2Id = await createAsset(fixture.context.algod, tokenCreatorAccount, 'Gating Token 2 [Required]', 'GATETK2', 10, 0)
...
describe('TokenGatingMultAssets', () => {
  ...
  beforeAll(async () => {
    ...
    for (let i = 0; i < 4; i += 1) {
      gatingTokens.push(await createAsset(fixture.context.algod, tokenCreatorAccount, `Gating Token ${i}`, `GATETK${i}`, 10, 0))
    }
...
describe('SaturatedValidator', () => {
  ...
  beforeAll(async () => {
    ...
    pools.push(await addStakingPool(fixture.context, validatorMasterClient, validatorId, 1, validatorOwnerAccount, poolMbr, poolInitMbr))
  ...
  test('testPenalties', async () => {
    ...
    if (diminishedRewards > BigInt(rewardAmount) - normalValidatorCommission) {
      consoleLogger.info(`reducing awards from ${diminishedRewards} to ${BigInt(rewardAmount) - normalValidatorCommission}`)
    ...
    expect(newFeeSinkBal.amount).toBeGreaterThanOrEqual(origFeeSinkBal.amount + (rewardAmount - Number(diminishedRewards)))
...
describe('StakeAddRemoveBugVerify', () => {
  ...
  test('addRemoveStakers', async () => {
    ...
    const stakeAmt = AlgoAmount.MicroAlgos(AlgoAmount.Algos(1000).microAlgos + AlgoAmount.MicroAlgos(Number(stakerMbr)).microAlgos)
...
describe('StakerMultiPoolAddRemoveBugVerify', () => {
  ...
  beforeAll(async () => {
    ...
    validatorIds.push(await addValidator(fixture.context, validatorMasterClient, validatorOwnerAccount, config, validatorMbr))
    poolKeys.push(
      await addStakingPool(fixture.context, validatorMasterClient, validatorIds[0], 1, validatorOwnerAccount, poolMbr, poolInitMbr),
    )
    validatorIds.push(await addValidator(fixture.context, validatorMasterClient, validatorOwnerAccount, config, validatorMbr))
    poolKeys.push(
      await addStakingPool(fixture.context, validatorMasterClient, validatorIds[1], 1, validatorOwnerAccount, poolMbr, poolInitMbr),
    )
  })
  ...
  test('stakeUnstakeReproduce', async () => {
    ...
    const val1Pool = new StakingPoolClient({ sender: stakerAccount, resolveBy: 'id', id: poolKeys[0].poolAppId }, fixture.context.algod)
  ...
  test(
    ...
    async () => {
      for (let i = 0; i < NumStakers + 1; i += 1) {
        ...
        expect(poolInfo.totalAlgoStaked).toEqual(BigInt(stakeAmount1.microAlgos - Number(stakerMbr)) * BigInt(i + 1))
...
describe('CoinFabrik Audit suggested extra tests', () => {
  describe('HI-01 Token Reward Calculation Inconsistent for Partial Stakers', () => {
    ...
    test('Token partial epoch rewards distributed should not affect subsequent distributions during the same epoch update', async () => {
      ...
      const stakeAmount = AlgoAmount.MicroAlgos(AlgoAmount.Algos(1000).microAlgos + AlgoAmount.MicroAlgos(Number(stakerMbr)).microAlgos)
  ...
  describe('ME-02 Incorrect Validator SunsettingOn Verification', () => {
    ...
    beforeAll(async () => {
      ...
      await addStakingPool(fixture.context, validatorMasterClient, validatorId, 1, validatorOwnerAccount, poolMbr, poolInitMbr)
      ...
      await validatorMasterClient
        .compose()
        .changeValidatorSunsetInfo({ validatorId, sunsettingOn: newSunset, sunsettingTo: validatorId }, { sender: validatorOwnerAccount })
  ...
  describe('ME-03 Incentivizing Pool Saturation for Staker Gain', () => {
    ...
    beforeAll(async () => {
      ...
      pool0Client = new StakingPoolClient({ sender: validatorOwnerAccount, resolveBy: 'id', id: pools[0].poolAppId }, fixture.context.algod)

      pool1Client = new StakingPoolClient({ sender: validatorOwnerAccount, resolveBy: 'id', id: pools[1].poolAppId }, fixture.context.algod)
    ...
    test('Saturation should not benefit stakers', async () => {
      ...
      expect((await getValidatorState(validatorMasterClient, validatorId)).totalAlgoStaked).toBeLessThan(constraints.AmtConsideredSaturated)
      ...
      const amountToSaturation = AlgoAmount.MicroAlgos(Number(constraints.AmtConsideredSaturated - validatorTotalStakeAfter + 1n))
      ...
      expect(validatorTotalStakeAfterSaturation).toEqual(validatorTotalStakeAfter + BigInt(amountToSaturation.microAlgos))
  ...
  describe('MI-05 Inconsistent Configuration Validation', () => {
    ...
    test.skip('SunsettingOn cannot be set before now', async () => {
      ...
      await expect(
        validatorMasterClient
          .compose()
          .changeValidatorSunsetInfo({ validatorId, sunsettingOn: badSunset, sunsettingTo: validatorId }, { sender: validatorOwnerAccount })
          .execute({ populateAppCallResources: true, suppressLog: true }),
      ).rejects.toThrowError()
    })
  })
